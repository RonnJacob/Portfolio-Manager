...
public class HowToInvestController<K> implements IHowToInvestController<K> {
  ...
  public void openPortfolioManager(IPortfolioManager<StockPortfolio> portfolioManagerModel) {
    ...
    while (true) {
      switch (scan.next()) {
        case "1": addPortfoliosToManager(portfolioManagerModel, scan);
                  openHomeScreen();
                  break;
      ...
      while (true) {
        switch (scan.next().toLowerCase()) {
          case "1": addToAppendable(selectedPFolio.getPortfolioData());
                    openPortfolioMenu();
                    break;
          case "2": buyStockShares(selectedPFolio, scan);
                    openPortfolioMenu();
                    break;
          case "3": addToAppendable("Enter date in format yyyy-mm-dd: \n");
